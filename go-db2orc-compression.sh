...
do
 java -Xmx2G \
  ...
  -l $DEMO_USER \
  -p $DEMO_PWD \
  -o "/storage/db2orc/compression/$TABLE-$value.orc" \
  -t "$TABLE" \
  --orc.compression "$value"

 if [ -f "/storage/db2orc/$TABLE-$value.orc" ]; then
    orc-metadata -v "/storage/db2orc/compression/$TABLE-$value.orc" > "/storage/db2orc/compression/$TABLE-$value-metadata.txt"
 fi
